...
ワークフロー・プロンプト・LoRA・画像サイズ・バッチ数の組を「ジョブ」としてリストに複数登録し、上から順番に自動実行できるページです。異なるワークフロー（例: `sdxl` と `anima`）を1回の操作でまとめて実行したい場合に使用します。

### 使い方

1. 「+ ジョブを追加」でジョブをリストに追加します（必要な数だけ繰り返します）
2. 一覧からジョブを選択すると、右側の編集パネルでそのジョブの内容（ワークフロー・プロンプト・画像サイズ・LoRA・バッチ数）を編集できます。各ジョブは Home ページと同様に個別に設定できます
...
登録したジョブの内容（ワークフロー・プロンプト・LoRA・画像サイズ・バッチ数）は、アプリのカレントディレクトリ直下の `queue_jobs.json` に保存され、再起動後も保持されます（`ComfyUIRunWorkflow_setting.json` とは別ファイルです）。`queue_jobs.json` が存在しない場合、ワークフローキューは空のリストから始まります。ただし実行ステータス・実行結果はセッション限りのため、再起動後は全ジョブが「未実行」から始まります（実行結果自体は `result_*.json` として残っています）。
