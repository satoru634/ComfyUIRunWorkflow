...
`ComfyUIRunWorkflow` のフェーズ2（GUI 実装）・フェーズ3（テンプレート配置）・フェーズ4（生成画像プレビュー表示）・フェーズ5（バッチ数指定）が完了・master マージ済み。
フェーズ6（WD14 Tagger 専用ページ）が `feature/tagger-page` ブランチで実装中。
...
- タグ付け履歴（tag_result_*.json）の DataPage への統合
- 実行履歴の永続化（SQLite 等）
