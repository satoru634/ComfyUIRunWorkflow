...
- ファイル名プレフィックスの上書き（未入力時はワークフローの設定値をそのまま使用）
- バッチ数指定（1〜10、実行ボタン左のバッチ数欄で指定した回数だけ連続実行）
- Home ページの設定内容（ワークフロー・プロンプト・画像サイズ・LoRA・バッチ数・ファイル名プレフィックス）を JSON でインポート・エクスポート
...
GUI 全体（画面文言・メッセージ・ナビゲーションメニュー・トレイメニューなど）を日本語/英語で切り替えられます。
...
ComfyUIRunWorkflow/   ← ソリューションルート
  ...
  ComfyUIRunWorkflowTests/ ← GUI テスト（252件）
